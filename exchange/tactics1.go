...
func (p *Tactics1) Init(date *TacticsData) {
...
func (p *Tactics1) GetID() int {
	return 1
}
func (p *Tactics1) Update() {
	if p.tactData == nil {
		return
	}
	p.denoiseMACD()
...
/*DIF突破0轴*/
func (p *Tactics1) crossOverByDIF() bool {
	mLen := len(*p.tactData.CurMACDs)
	if mLen <= 0 {
		log.Infoln("没有可用的MACD数据")
		return false
